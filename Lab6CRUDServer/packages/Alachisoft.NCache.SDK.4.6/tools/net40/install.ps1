...
if($osType -eq "64-bit")
{
	$project.Properties.Item("PostBuildEvent").Value += "`r`nxcopy `"$installPath\additionalLib\Oracle40\x64\Oracle.ManagedDataAccess.dll`" `"`$`(ProjectDir`)\bin\$`(ConfigurationName`)\`" /Y /I";
}
elseif($osType -eq "32-bit")
{
	$project.Properties.Item("PostBuildEvent").Value += "`r`nxcopy `"$installPath\additionalLib\Oracle40\x32\Oracle.ManagedDataAccess.dll`" `"`$`(ProjectDir`)\bin\$`(ConfigurationName`)\`" /Y /I";
}
else
{
